...
const MatchPrediction = () => {
   ...
   const [activeTab, setActiveTab] = useState(localStorage.getItem("matchTab") || "upcoming");
   ...
   const saveActiveTab = (value) => {
      setActiveTab(value);
      localStorage.setItem('matchTab', value);
   }
      ...
      <div className="min-h-screen bg-gray-900 text-gray-200 py-8 px-4 max-w-6xl mx-auto">
         ...
         <div className="flex gap-4 mb-6 border-b border-gray-700">
            <button
               ...
               onClick={() => saveActiveTab("upcoming")}
            >
               Upcoming Matches
            </button>
            <button
                        ...
                        <div key={match._id} onClick={() => handleMatchSelection(match)} className="p-4 bg-gray-700 rounded-lg">
                           ...
                           <div className="flex items-center justify-center">
                              <div className="flex items-center gap-3">
                                 <img src={match.teamOneImage} alt={match.teamOneName} className="w-10 h-10" />
                                 <span className="text-lg font-semibold">{match.teamOneName}</span>
                              </div>
                              <span className="text-gray-400 px-5">vs</span>
                              <div className="flex items-center gap-3">
                                 <span className="text-lg font-semibold">{match.teamTwoName}</span>
                                 <img src={match.teamTwoImage} alt={match.teamTwoName} className="w-10 h-10" />
                              </div>
                           </div>
                                       ...
                                       {Object.entries(matchPredictions).map(([team, users]) => (
                                          <tr key={team} className="text-center bg-gray-700">
                                             ...
                                             <td className="border border-gray-600 px-4 py-2">
                                                {users.length > 0 ? users.map(user => user).join(", ") : "No votes"}
                                             </td>
                                          </tr>
                                       ))}
                                    </tbody>
